...
def run_all_checks() -> dict[str, list[CheckResult]]:
    ...
    cat1.append(
        (
            dir_exists("docs") and any((REPO_ROOT / "docs").iterdir())
            if dir_exists("docs")
            else False,
            "docs/ directory with structured subdirectories",
        )
    )
    cat1.append((file_exists("PLANS.md"), "ExecPlan template (PLANS.md) exists"))
    cat1.append((dir_exists("docs/design-docs"), "Design docs directory exists"))
    cat1.append(
        (
    ...
    cat1.append((dir_exists("docs/references"), "Reference docs directory exists"))
    ...
    cat2.append(
        (
            file_exists("scripts/validate.sh")
            and file_is_executable("scripts/validate.sh"),
    ...
    cat2.append(
        (
            file_contains_pattern(
                "scripts/validate.sh", r"ruff format|prettier|black|rustfmt"
            ),
            "Format gate exists",
        )
    )
    cat2.append(
        (file_exists("scripts/check_imports.py"), "Import boundary checker exists")
    )
    cat2.append(
        (
    ...
    cat2.append(
        (file_exists("scripts/check_architecture.py"), "Architecture checker exists")
    )
    cat2.append(
        (dir_exists(".github/workflows"), "CI workflow exists (.github/workflows/)")
    )
    cat2.append((_check_ci_blocks_merge(), "CI blocks merge (required status checks)"))
    ...
    cat3.append(
        (
            file_exists("backend/pyproject.toml")
            and file_contains_pattern("backend/pyproject.toml", r"pytest")
            or file_exists("backend/setup.cfg")
            and file_contains_pattern("backend/setup.cfg", r"pytest")
            or file_exists("pytest.ini")
            or file_exists("package.json")
            and file_contains_pattern("package.json", r"jest|vitest")
    ...
    cat3.append(
        (
            any(
                file_exists(f)
                for f in [
                    "scripts/check_ui_legibility.sh",
    ...
    cat4.append(
        (file_exists("scripts/boot_worktree.sh"), "Per-worktree boot script exists")
    )
    cat4.append(
        (
...
def print_scorecard(results: dict[str, list[CheckResult]]) -> None:
    ...
    print(
        f"  Grade: {c(gc, f'  {grade}  ')}   ({total_passed}/{total_checks} checks passed)"
    )
    print()
