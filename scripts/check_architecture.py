    ...
    for f in py_files:
        ...
        if line_count > MAX_LINES:
            violations.append(
                ...
                f"File exceeds {MAX_LINES} lines. Fix: identify the distinct concerns in this file "
                f"(e.g., data validation vs business logic vs HTTP handling), then extract each into "
                f"its own file in the appropriate module directory. A file should do one thing. "
                f"See docs/design-docs/core-beliefs.md #5 (One Service Does One Thing Well)."
            )
                ...
                for pattern in DB_IMPORT_PATTERNS:
                    if pattern in node.module:
                        violations.append(
                            ...
                            f"Fix: create a function in {DB_MODULE}/ that wraps this database call, "
                            f"then import that function instead of '{pattern}' directly. "
                    ...
                    for pattern in DB_IMPORT_PATTERNS:
                        if pattern in alias.name:
                            violations.append(
                                f"  {rel}:{node.lineno} — "
                                f"Direct DB import outside {DB_MODULE}/. "
                                f"Fix: move this database call into a function in {DB_MODULE}/, "
                                f"then import that function here instead."
                            )
