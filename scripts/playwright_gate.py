...
def run_default_checks(frontend_url, snapshot_dir):
    ...
    with sync_playwright() as p:
        ...
        results.append(
            (
                f"No console errors (found {len(console_errors)})",
        ...
        for path in ["/login", "/dashboard", "/agents/new"]:
            try:
                resp = page.goto(
                    f"{frontend_url}{path}", wait_until="networkidle", timeout=10000
                )
...
def main():
    ...
    print()
    print(
        f"UI Features: {passed} passed, {failed} failed, {skipped} skipped, {len(ui_features)} total"
    )
