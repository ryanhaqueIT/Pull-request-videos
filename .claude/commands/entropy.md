...
3. Search for entropy markers in source code:
   - `grep -rn "TODO\|FIXME\|HACK\|XXX\|NOCOMMIT" --include="*.py" --include="*.ts" --include="*.tsx" --include="*.js" | head -30`
4. Check for orphan files: find source files that are not imported by any other file in the project. Ignore `__init__.py`, test files, and scripts/.
5. Check for dead code: find functions/classes defined that are never referenced elsewhere (best effort — check with grep).
6. Check for pattern drift:
   - Are all routers registered in the main app file?
